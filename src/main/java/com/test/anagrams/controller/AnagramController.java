...
public class AnagramController {
    ...
    public ResponseEntity<AllAnagramsDTO> getAllAnagrams(@PathVariable String s1) {
        if (Strings.isNullOrEmpty(s1) || s1.length() <= 1)
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();

        AllAnagramsDTO allAnagrams = new AllAnagramsDTO();
        allAnagrams.setAnagrams(anagramService.getAllAnagramStrings(s1));
        return ResponseEntity.status(HttpStatus.OK)
                .body(allAnagrams);
    }
